<div>
    ...
    <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-8">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Productos</h1>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Gestiona el inventario de productos de tu tienda.
            </p>
        </div>
    ...
    <div class="md:hidden space-y-4 mb-6">
        @forelse ($products as $product)
            <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-4">
                <div class="flex justify-between items-start mb-3">
                    ...
                    <span
                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border {{ $product->status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20' : 'bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20' }}">
                        <span
                            class="w-1.5 h-1.5 mr-1.5 rounded-full {{ $product->status === 'active' ? 'bg-emerald-500' : 'bg-red-500' }}"></span>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <span class="text-xs text-zinc-500 dark:text-zinc-400 block">Precio Venta</span>
                        <span class="text-lg font-bold text-zinc-900 dark:text-white">
                            ${{ number_format($product->sale_price, 2) }}
                        </span>
                    </div>
                    <div>
                        <span class="text-xs text-zinc-500 dark:text-zinc-400 block">Categoría</span>
                        <span
                            class="text-sm font-medium text-zinc-900 dark:text-zinc-200">{{ $product->category->category_name ?? '-' }}</span>
                        @if($product->subcategory)
                            <span
                                class="text-xs text-zinc-500 dark:text-zinc-400 block">{{ $product->subcategory->subcategory_name }}</span>
                        @endif
                    </div>
                </div>
        ...
        @empty
            <div class="text-center py-8 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700">
                ...
                <p class="mt-1 text-xs text-zinc-400">Intenta ajustar tu búsqueda o crea un nuevo producto.</p>
            </div>
        @endforelse
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700 bg-white dark:bg-zinc-900">
                    @forelse ($products as $product)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border {{ $product->status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20' : 'bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20' }}">
                                    <span
                                        class="w-1.5 h-1.5 mr-1.5 rounded-full {{ $product->status === 'active' ? 'bg-emerald-500' : 'bg-red-500' }}"></span>
                                    {{ ucfirst($product->status) }}
                                </span>
    ...
    <livewire:inventory.product.create />
    <livewire:inventory.product.edit />

    {{-- Delete Modal --}}
    <flux:modal name="delete-product" class="md:w-[400px] !p-0 overflow-hidden bg-white dark:bg-zinc-900 rounded-xl">
        <div class="px-4 py-5 sm:p-6">
            <div class="flex items-start">
                ...
                <div class="ml-4 mt-0.5 text-left">
                    <h3 class="text-base font-semibold leading-6 text-zinc-900 dark:text-zinc-100">Eliminar Producto
                    </h3>
                    <div class="mt-2">
