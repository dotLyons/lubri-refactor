<div>
    ...
    <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-8">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Subcategorías de Productos</h1>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Gestiona y organiza las subcategorías de tu
                inventario.</p>
        </div>
        <flux:modal.trigger name="create-subcategory">
            <button type="button"
    ...
    <div class="md:hidden space-y-4 mb-6">
                ...
                <div class="flex justify-between items-start mb-3">
                    <div>
                        <h3 class="text-base font-semibold text-zinc-900 dark:text-white">
                            {{ $subcategory->subcategory_name }}</h3>
                        <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1">{{ $subcategory->description ?? '-' }}</p>
                    </div>
                    <span
                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border {{ $subcategory->status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20' : 'bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20' }}">
                        <span
                            class="w-1.5 h-1.5 mr-1.5 rounded-full {{ $subcategory->status === 'active' ? 'bg-emerald-500' : 'bg-red-500' }}"></span>
                        {{ $subcategory->status === 'active' ? 'Activo' : 'Inactivo' }}
                    </span>
                </div>

                <div class="grid grid-cols-2 gap-3 pt-3 border-t border-zinc-100 dark:border-zinc-800">
                    <button wire:click="$dispatch('edit-subcategory', { id: {{ $subcategory->id }} })"
                    ...
                    <flux:modal.trigger name="delete-subcategory">
                        <button wire:click="confirmDelete({{ $subcategory->id }})"
        ...
        @empty
            <div class="text-center py-8 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700">
                <p class="text-sm text-zinc-500">No se encontraron subcategorías</p>
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700 bg-white dark:bg-zinc-900">
                    @forelse ($subcategories as $subcategory)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border {{ $subcategory->status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20' : 'bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20' }}">
                                    <span
                                        class="w-1.5 h-1.5 mr-1.5 rounded-full {{ $subcategory->status === 'active' ? 'bg-emerald-500' : 'bg-red-500' }}"></span>
                                    {{ $subcategory->status === 'active' ? 'Activo' : 'Inactivo' }}
                                </span>
    ...
    <livewire:inventory.subcategories.create />
    <livewire:inventory.subcategories.edit />

    {{-- Delete Modal --}}
    <flux:modal name="delete-subcategory"
        class="md:w-[400px] !p-0 overflow-hidden bg-white dark:bg-zinc-900 rounded-xl">
        <div class="px-4 py-5 sm:p-6">
            <div class="flex items-start">
                ...
                <div class="ml-4 mt-0.5 text-left">
                    <h3 class="text-base font-semibold leading-6 text-zinc-900 dark:text-zinc-100">Eliminar Subcategoría
                    </h3>
                    <div class="mt-2">
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">
                            ¿Estás seguro de que deseas eliminar esta subcategoría? Esta acción es irreversible.
                        </p>
                    </div>
                </div>
            </div>
