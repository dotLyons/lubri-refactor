<div>
    <flux:modal name="create-customer" class="md:w-3/4 max-w-4xl">
        <form wire:submit="save" class="space-y-6">
            <div class="flex items-center p-6 border-b border-zinc-200 dark:border-zinc-800">
                <h3 class="text-lg font-bold text-zinc-900 dark:text-white">Registrar Nuevo Cliente</h3>
            </div>
                ...
                <div class="space-y-4 pt-4">
                    ...
                    @foreach($vehicles as $index => $vehicle)
                        <div class="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-xl border border-zinc-200 dark:border-zinc-700 relative">
                            @if(count($vehicles) > 1)
                                <button type="button" wire:click="removeVehicle({{ $index }})" class="absolute top-2 right-2 text-zinc-400 hover:text-red-500">
            ...
            <div class="p-6 pt-2 bg-zinc-50 dark:bg-zinc-800/50 border-t border-zinc-200 dark:border-zinc-800 flex justify-end gap-3 rounded-b-xl">
                ...
                <flux:button type="submit" variant="primary" icon="check">Guardar Cliente</flux:button>
            </div>
        </form>
    </flux:modal>
</div>
